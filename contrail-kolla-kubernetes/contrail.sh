...
then
  ...
  ${KUBECTL} label nodes stacknamecontrail_hostname opencontrail.org/controller=true
  cd contrail-docker/kubernetes/manifests/
  sed -i 's/10.84.13.7/contrail_ip/g'  contrail-host-ubuntu.yaml
  sed -i '/opscenter_ip/d' contrail-host-ubuntu.yaml
